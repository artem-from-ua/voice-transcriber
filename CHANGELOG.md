...
- Validated on the first 12 min of the 48-min reference: A+C combo applied with `--merge-split-filler-bias-tie-ms 120 --merge-split-filler-lang en --merge-split-deadband-ms 100` correctly recovered both the canonical leading-filler case (deadband) and the `"yes"` back-channel case (filler-bias) with 0 regressions on the other 1404 words. Full-file validation in the PR body.
