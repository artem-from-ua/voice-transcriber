...
| `--llm-model` | str | `mlx-community/gemma-3-12b-it-qat-4bit` | LLM model id, as known to LM Studio. |
| `--llm-base-url` | URL | `http://localhost:1234/v1` | LM Studio endpoint. |
...
# 5) Custom output path and a remote LM Studio
uv run voice transcribe a.m4a \
  -o ~/notes/2026-05-11-call.md \
  --llm-base-url http://desktop.local:1234/v1
```
