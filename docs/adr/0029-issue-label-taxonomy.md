...
**Color per axis, not per value.** All `type:*` share `#cccccc` (with `type:bug` as an exception in `#d73a4a` red so bugs stand out in lists). All `area:*` share `#74a892` (muted sage). All `stage:*` share `#0052cc`. All `by:*` share `#cccccc` (provenance is metadata, not primary signal). `priority:*` uses a red→grey gradient (the only axis where the gradient carries meaning — urgency); `priority:critical` shares the exact same red `#d73a4a` as `type:bug` so the two highest-alarm signals look identical at a glance. The prefix carries identity; the color carries the axis. A per-stage blue gradient was considered (upstream-light → downstream-dark) and rejected after first use: multi-stage issues read as visual noise; one calm color is easier to scan.
