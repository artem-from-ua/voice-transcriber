...
**Cache miss** (`whisper_asr._verify_model_cached`): `huggingface_hub.try_to_load_from_cache(repo_id, "config.json")`. Returns `None` ⇒ raise `AsrError` with the message "Whisper model ... is not in the Hugging Face cache. Run `voice download-whisper` once to fetch it (~3 GB)." No silent fetches inside `transcribe`.
