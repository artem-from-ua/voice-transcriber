...
The single most important finding sits in row B4. Adding presence boost **without** bandpass first **catastrophically breaks ASR**: the Russian-glyph rate jumps from 5.1 % (default `agc`) to 63 %. Inspection of the 29 flagged segments confirms this is real language drift, not stray letters — VibeVoice renders whole Ukrainian utterances as fluent sentences in a different language.
