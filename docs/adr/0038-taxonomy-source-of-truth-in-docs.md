...
- **Take over the Dependabot labels** (`dependencies`, `python:uv`). Rejected: another tool creates and maintains them. They are recorded as `keep` in the legacy mapping and are never auto-deleted — the plugin reports them as undeclared and leaves them alone.
