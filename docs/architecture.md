...
6. **speech2text** — `whisper_asr.transcribe(cleaned_wav, language=effective_language)` → `AsrSegment[]` via `mlx-whisper` against `mlx-community/whisper-large-v3-mlx`. The `effective_language` value is either the CLI `--language` override or the ISO code computed by `[4] lang_detect`. Whisper produces no speaker hint; speaker labels come entirely from pyannote turns at stage 7 (merge). Short audio (≤ `ASR_CHUNK_THRESHOLD_S = 600 s`) is transcribed in one call; longer audio is sliced in Python into 8-minute chunks with 5 s overlap and re-stitched with timestamp offsets — the per-call MLX peak is bounded by chunk size, not input duration. See [ADR 0017](adr/0017-whisper-asr-backend.md) for how this backend was chosen, [ADR 0021](adr/0021-remove-vibevoice-backend.md) for why it is now the only backend, and [ADR 0031](adr/0031-chunked-asr.md) for the chunking rationale.
...
When `--user-context "..."` is supplied on the CLI, its text is prepended to the system prompt of every LLM-driven stage (8 proofread, 9 identify_speakers, 10 speech_structure, 11 safe_speech, 12 speech_summary) as a short "the user described this conversation as: ..." block — Ukrainian or English wording chosen per stage's prompt language. Default is `None`: stages run with their unmodified system prompts. ASR (stage 6) is not affected — Whisper `initial_prompt` is tracked separately. See [ADR 0033](adr/0033-user-context-system-prompt-prefix.md).
